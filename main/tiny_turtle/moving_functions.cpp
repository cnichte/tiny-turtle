...
//---------------------------------------------------------------------------- check bumper switches and spin free if there's an obstacle
void bounce() {

  bool penState = drawing;

  if (!digitalRead(SWITCH_F)) { direction = -1; }  //                          change direction, depending on which bumper was hit
  if (!digitalRead(SWITCH_B)) { direction = 1; }

  while (checkBumper()) {  //                                                  if a bumper was hit - repeat until all bumpers are free
    penUp();
    int randomTurnDirection = random(2) ? -1 : 1;
    while (checkBumper()) {  //                                                spin free (until no bumper switch is pressed) in random direction
      turn(randomTurnDirection);
    }
    turn(randomTurnDirection * random(5, 45));  //                             spin a bit more
  }
  if (penState) { penDown(); }  //                                             reset pen state
}
  ...
  while (distanceTraveled < finalDistance) {
    ...
    for (long k = 0; k < requiredOuterStepsForSegment; k++) {
      ...
      if (stopAtSensor && sensorCollision) {
        ...
        finalDistance = std::min(distanceTraveled + sensorOffset, finalDistance);  //             recalculate target distance
